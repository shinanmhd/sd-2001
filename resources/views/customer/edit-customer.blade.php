            ...
            <div class="px-4 py-6 sm:px-0">
                ...
                <div class="grid place-items-center">
                    <div class="bg-white w-full border p-12">
                        <h1 class="text-xl font-semibold mb-6">Please fill in the information for the customer</h1>
                        <form class="mt-6" method="post" action="{{ route('customer.update', ['customer' => $customer]) }}">
                            @csrf
                            @method('PUT')
                            <div class="flex justify-between gap-3 mb-6">
                                <span class="w-1/2 flex flex-col">
                                    ...
                                    <input id="customer_name" type="text" name="customer_name" placeholder="Customer Name" autocomplete="given-name" class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner" value="{{ $customer->name }}" />
                                ...
                                <span class="w-1/2 flex flex-col">
                                    ...
                                    <input id="is_subscribed_to_newsletter" type="checkbox" name="is_subscribed_to_newsletter" class="mt-2 p-2" {{ $customer->is_subscribed_to_newsletter ? 'checked' : '' }} />
                                </span>
                            </div>
                            ...
                            <div class="w-full flex flex-col mb-6">
                                ...
                                <select name="membership_type" id="membership_type" class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner">
                                    ...
                                    @foreach ($membershipTypes as $membershipType)
                                        <option value="{{ $membershipType->id }}" @if ($customer->membership_type_id == $membershipType->id) selected @endif>{{ $membershipType->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="w-full flex flex-col mb-6">
                                ...
                                <input id="date_of_birth" type="date" name="date_of_birth" class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner" value="{{ $customer->date_of_birth }}" />
